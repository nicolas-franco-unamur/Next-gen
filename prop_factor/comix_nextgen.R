...
#use "RDS" if the re-aggregate data must be used ('file PCR_REAGGREGATE_DATA.RDS')
PCR_file <- "RDS"
#use a csv name if the non aggregate data must be used
#This file must be requested from Sciensano
#PCR_file <- "COVID19BE_AGE_POSTCODE_SEX_LAB.csv"

#age groups to consider (last group going up to infinity) - Do not change if using re-aggregate RDS file
...
Wavegroupsizemin <- 0
#Number of samples for bootstrap (>1)
N_bs <- 10    #200
...
for(group in 1:Wavegroupsnum){
  susinflist[[group]] <- vector("list", length = N_bs)  
...
for(bs in 1:N_bs){
  ...
  for(group in 1:Wavegroupsnum){
    susinflist[[group]][[bs]] <- susinfvec[[group]]
  }
...
for(bs in 1:N_bs){
  ...
  for(i in Wavemin:Wavemax){
    if(exists("Fixed_Susceptibility")){
      ...
      Susceptibility <- susinflist[[Wavegroups[[i-Wavemin+1]]]][[bs]]
    }
    if(exists("Fixed_Infectiousness")){
      Infectiousness <- Fixed_Infectiousness
    } else {
      Infectiousness <- susinflist[[Wavegroups[[i-Wavemin+1]]]][[bs]]
    }
...
for(i in Wavemin:Wavemax){
  ...
  comix_nextgenfinal[[i]] <-  bind_rows(comix_nextgenfinal[[i]]$bs)  %>% group_by(ageclass) %>% summarise(nextgenmean = mean(nextgen), nextgenmedian = median(nextgen),  nextgensd = sd(nextgen), nextgenlower = unname(quantile(nextgen,0.025)), nextgenupper = unname(quantile(nextgen,0.975)), .groups = 'drop')
...
for(group in 1:Wavegroupsnum){
  ...
  susinfdataframe[[group]] <- susinfdataframe[[group]]  %>% group_by(ageclass) %>% summarise(mean = mean(values), median = median(values),  sd = sd(values), lower = unname(quantile(values,0.025)), upper = unname(quantile(values,0.975)), .groups = 'drop')
...
# #plot sus/inf - different layout (for non time-evolutive)
# plotsusinf <- ggplot(commondataframe) +
#   geom_point(aes(x = wave, y=mean,group = 1,colour = factor(wave)), size=2*1.5)   +
#   geom_line(aes(x = wave, y=mean,group = 1), colour="darkorange",linetype = "solid", size=1)   +
#   geom_linerange(aes(x = wave, y = mean,ymin=lower, ymax=upper,colour = factor(wave)), size=0.7*1.5) +
#   geom_ribbon(aes(x = wave,ymin=lower, ymax=upper,group = 1) ,fill="darkorange", alpha=0.2) +
#   facet_wrap(~ageclass, strip.position="bottom", ncol=length(Age_breaks)) +
#   geom_hline(yintercept=1,color = "grey") +
#   theme_minimal()+
...
plotrelincid
ggsave("fig_relincid.eps", device=cairo_ps, width = 9, height = 6*9/11)
plotsusinf
ggsave("fig_susinf.eps", device=cairo_ps, fallback_resolution = 1200, width = 9*0.85, height = 6*0.85)
#ggsave("fig_susinf.eps", device=cairo_ps, width = 9*0.85*0.65, height = 6*0.85*0.65)


pdf("fig_relincid.pdf", width = 9, height = 6*9/11)
print(plotrelincid);
dev.off() 
pdf("fig_susinf.pdf", width = 9*0.85, height = 6*0.85)
print(plotsusinf);
dev.off()
save.image("nextgen.RData") 
